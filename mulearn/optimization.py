...
class CooperSolver(Solver):
    ...
    Using this class requires that Cooper is installed."""

    default_values = {"initial_values": "random",
                      "init_bound": 0.1,
                      "n_iter": 1000,
                      "optimizer": cooper.optim.ExtraAdam
                                   if torch_ok else None,  # noqa
                      "tracker": tqdm.trange if tqdm_ok else range,
                      "lr": 1e-3,
                      "atol": 1e-3,
                      "device": "cpu"}
    ...
    def __init__(self,
                 ...
                 optimizer=default_values["optimizer"],
                 tracker=default_values["tracker"],
                 lr = default_values["lr"],
                 atol=default_values["atol"],
                 device=default_values["device"]): 
        """Build an object of type CooperSolver.
        ...
        :param n_iter: Number of iterations of the optimization process,
          defaults to 1000.
        :type n_iter: `int`
        :param optimizer: Optimization algorithm to be used, defaults to ExtraAdam
          if cooper is available, to None otherwise.
        :type optimizer: :class:`cooper.optim.ExtraAdam`
        :param tracker: Tool to graphically depict the optimization progress,
          defaults to `tqdm.trange` if tqdm is available, to `range` (i.e., no
          graphical tool) otherwise.
        :type tracker: `object`
        ...
        :type lr: `float`
        :param atol: Tolerance for the violation of optimization problem constraints.
        :type atol: `float`
        :param device: Device to be used for tensor computations, defaults to `cpu`
        :type device: `str`
        """
    ...
    class mulearnCMP(cooper.ConstrainedMinimizationProblem):
        ...
        def closure(self, k, mus, C, chis):
            ...
            # Compute the violation of the constraints functions
    ...
    def solve_problem(self, xs, mus, c, k):
        ...
        Build and solve the constrained optimization problem at the basis
        of the fuzzy learning procedure using the cooper API.
        ...
        #np.random.seed(42)
        if self.initial_values == 'random':
            a = np.random.uniform(-self.init_bound, self.init_bound, len(mus))
        else:
            a = self.initial_values

        # Define the primal variables and optimizer
        chis = torch.nn.Parameter(torch.tensor(a, dtype=torch.float64, requires_grad=True))
        primal_optimizer = self.optimizer([chis], lr=self.lr)

        # Define the dual optimizer, not fully instantiated
        dual_optimizer = cooper.optim.partial_optimizer(self.optimizer, lr=self.lr)

        # Wrap the optimizers together
        ...
        while (i < self.n_iter):
            ...
            constr1 = self.almost_negative(formulation.cmp.state.ineq_defect[0], atol=self.atol)
            constr2 = self.almost_negative(formulation.cmp.state.ineq_defect[1], atol=self.atol)
            constr3 = torch.isclose(torch.sum(chis), const1, atol=self.atol)
    
            gap = formulation.cmp.state.loss - formulation.dual_parameters[1]

            # End training if solution is optimal and constraint are satisfied with defined tolerance (self.atol)
        ...
        #if optim_chis == None:
        #    raise ValueError('optim not found, retry with greater tolerance')

        if not opt_found:
            logger.warning('cooper: optimal values not found')
            #TODO raise valueerror

        if self.device == 'gpu':
            return chis.detach().cpu().numpy()
        
        return chis.detach().numpy()


    def __repr__(self):
        obj_repr = f"CooperSolver("

        for a in ("initial_values", "init_bound", "n_iter",
                  "optimizer", "tracker", "lr", "atol", "device"):
            if self.__getattribute__(a) != self.default_values[a]:
                obj_repr += f", {a}={self.__getattribute__(a)}"
        return obj_repr + ")"
